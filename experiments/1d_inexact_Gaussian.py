...
for service_shape in [0.8, 0.6, 0.4, 0.2]:
    ...
    _, _, theta_list = sgd_estimation(lindley, x_sample_large, KS, theta_init, 
...
for service_shape in [1.0, 0.8, 0.6, 0.4, 0.2]:
    ...
    for k in tqdm(range(macro_count)):
        ...
        if k == 0:
            ...
            # Save the plot
            os.makedirs('./results/plots/1d_inexact_Gaussian', exist_ok=True)
            plt.savefig(f'./results/plots/1d_inexact_Gaussian/1d_inexact_Gaussian_alpha={service_shape}_CI.png')
            plt.close()          
            
        df['Average CPU Time'][index] += (time.time() - start_time) / macro_count
    
    # Create the directory if it doesn't exist
    os.makedirs('./results/dataframes/1d_inexact_Gaussian', exist_ok=True)

    # Save the dataframe to a CSV file
    pd.DataFrame(df).to_csv(f'./results/dataframes/1d_inexact_Gaussian/results_alpha={service_shape}.csv', index=False)
    index += 1
...
# Save the dataframe to a CSV file
pd.DataFrame(df).to_csv('./results/dataframes/1d_inexact_Gaussian/results.csv', index=False)