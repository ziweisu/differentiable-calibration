...
for m in [10, 50, 100]:
    ...
    for k in tqdm(range(macro_count)):
        ...
        if k == 0:
            ...
            # Save the plot
            os.makedirs('./results/plots/2d_inexact_stat_Gaussian_t_adjust', exist_ok=True)
            plt.savefig(f'./results/plots/2d_inexact_stat_Gaussian_t_adjust/2d_inexact_stat_Gaussian_m={m}_CI.png')
            plt.close()            
            ...
            # Save the plot
            os.makedirs('./results/plots/2d_inexact_stat_Gaussian_t_adjust', exist_ok=True)
            plt.savefig(f'./results/plots/2d_inexact_stat_Gaussian_t_adjust/2d_inexact_stat_Gaussian_m={m}_conv.png')
            plt.close()
    ...
    os.makedirs('./results/dataframes/2d_inexact_stat_Gaussian_t_adjust', exist_ok=True)

    # Save the dataframe to a CSV file
    pd.DataFrame(df).to_csv(f'./results/dataframes/2d_inexact_stat_Gaussian_t_adjust/results_m={m}.csv', index=False)
    index += 1
...
# Save the dataframe to a CSV file
pd.DataFrame(df).to_csv('./results/dataframes/2d_inexact_stat_Gaussian_t_adjust/results.csv', index=False)