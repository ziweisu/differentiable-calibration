...
for epsilon in [0.01, 0.05, 0.1, 0.2, 0.5]:
    ...
    for k in tqdm(range(macro_count)):
        ...
        if k == 0:         
            ...
            # Save the plot
            os.makedirs('./results/plots/2d_inexact_stat_noise', exist_ok=True)
            plt.savefig(f'./results/plots/2d_inexact_stat_noise/2d_inexact_stat_noise_epsilon={epsilon}_conv.png')
            plt.close()

        df['Average CPU Time'][index] += (time.time() - start_time) / macro_count
    
    # Create the directory if it doesn't exist
    os.makedirs('./results/dataframes/2d_inexact_stat_noise', exist_ok=True)

    # Save the dataframe to a CSV file
    pd.DataFrame(df).to_csv(f'./results/dataframes/2d_inexact_stat_noise/results_epsilon={epsilon}.csv', index=False)
    index += 1
...
# Save the dataframe to a CSV file
pd.DataFrame(df).to_csv('./results/dataframes/2d_inexact_stat_noise/results.csv', index=False)