...
number_epochs = [2500, 2000, 1500, 1000, 1000]
index = 0
service_shape = 0.8
for n in n_list:
    df['n'][index] = n
    ...
    for k in tqdm(range(macro_count)):
        ...
        if k == 0:
            ...
            plt.savefig(f'./results/plots/2d_inexact_stat/2d_inexact_stat_n={n}_ada_CI.png')
            plt.close()            
            ...
            # Save the plot
            os.makedirs('./results/plots/2d_inexact_stat', exist_ok=True)
            plt.savefig(f'./results/plots/2d_inexact_stat/2d_inexact_stat_n={n}_ada_conv.png')
            plt.close()
    ...
    pd.DataFrame(df).to_csv(f'./results/dataframes/2d_inexact_stat/results_t_adjust_n={n}_ada.csv', index=False)
    index += 1
