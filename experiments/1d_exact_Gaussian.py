...
for m in [10]:
    ...
    df['n'][index] = m
    df['m'][index] = m
    df['Service_shape'][index] = 1
    theta_opt = df['theta_opt'][index]
    x_sample = lindley(service_rate=1.2, service_shape_parameter=1, replications=m, device=device)
    for k in tqdm(range(macro_count)):
        start_time = time.time()
        theta_tilde, _, para_list = sgd_estimation(lindley, x_sample, KS, theta_init, 
        ...
        ci_lower, ci_upper, width, var_hat = conf_int(lindley, x_sample, KS, theta_tilde, Gaussian, device=device)
        if np.isnan(width):
            nan_count += 1
        else:
            df['Width'][index] += width
        ...
        if k == 0:
            ...
            ax.set_title(f'Confidence Interval (m={m})')
            ax.legend()

            # Save the plot
            os.makedirs('./results/plots/1d_exact_Gaussian', exist_ok=True)
            plt.savefig(f'./results/plots/1d_exact_Gaussian/1d_exact_Gaussian_m={m}_CI.png')
            plt.close()          
            
        df['Average CPU Time'][index] += (time.time() - start_time) / macro_count
    df['Width'][index] /= (macro_count-nan_count)
    # Create the directory if it doesn't exist
    os.makedirs('./results/dataframes/1d_exact_Gaussian', exist_ok=True)

    # Save the dataframe to a CSV file
    pd.DataFrame(df).to_csv(f'./results/dataframes/1d_exact_Gaussian/results_m={m}.csv', index=False)
    index += 1
...
# Save the dataframe to a CSV file
pd.DataFrame(df).to_csv('./results/dataframes/1d_exact_Gaussian/results.csv', index=False)