...
index = 0
n_list = [2, 10, 50, 100]
number_epochs = [2500, 2000, 1500, 1000]
m = 1000
...
for n in n_list:
    ...
    df['number of epochs'][index] = number_epochs[index]
    ...
    for k in tqdm(range(macro_count)):
        ...
        theta_tilde, _, para_list = sgd_estimation(lindley, x_sample, KS, theta_init, service_shape_parameter=1, 
                                                   arrival_shape_parameter=0.5, simulation_replications=n, number_epochs=df['number of epochs'][index], lr_init=1, device=device)
        ...
        if k == 0:
            ...
            ax.set_title(f'Confidence Set (m={m}, n={n})')
            ax.legend()
            
            # Save the plot
            os.makedirs('./results/plots/2d_inexact_stat_Gaussian', exist_ok=True)
            plt.savefig(f'./results/plots/2d_inexact_stat_Gaussian/2d_inexact_stat_Gaussian_n={n}_ada_CI.png')
            plt.close()            
            ...
            ax.set_xlabel('Service Rate')
            ax.set_ylabel('Arrival Rate')
            ax.set_title(f'Parameter Convergence(m={m}, n={n})')
            ax.legend()
             
            # Save the plot
            os.makedirs('./results/plots/2d_inexact_stat_Gaussian', exist_ok=True)
            plt.savefig(f'./results/plots/2d_inexact_stat_Gaussian/2d_inexact_stat_Gaussian_n={n}_ada_conv.png')
            plt.close()
    ...
    pd.DataFrame(df).to_csv(f'./results/dataframes/2d_inexact_stat_Gaussian/results_n={n}_ada.csv', index=False)
    index += 1
