...
df = {'N': [0, 0, 0, 0, 0], 'm': [0, 0, 0, 0, 0], 'MSE, posterior mean': [0, 0, 0, 0, 0],
      'CovProb, Credible Set': [0, 0, 0, 0, 0], 'Width, Credible Set': [0, 0, 0, 0, 0], 'cpu time': [0, 0, 0, 0, 0]}
macro_count = 10
...
for m in [10, 50, 100, 500, 1000]:
    x_sample = lindley(1.2, replications=m, device=device)
    df['N'][index] = N
    df['m'][index] = m
    ...
    for k in tqdm(range(macro_count)):
        time_now = time.time()
        search_range = torch.from_numpy(np.random.uniform(0.5, 2.5, N)).to(device)
        ...
        if k == 0:
            ...
            os.makedirs('./results/plots/1d_exact_MMD_ABC', exist_ok=True)
            
            # Save the plot
            plt.savefig(f'./results/plots/1d_exact_MMD_ABC/1d_exact_MMD_ABC_m={m}.png')
            plt.close()
        df['cpu time'][index] += (time.time() - time_now) / macro_count
...
# Create the directory if it doesn't exist
os.makedirs('./results/dataframes/1d_exact_MMD_ABC', exist_ok=True)

# Save the dataframe to a CSV file
pd.DataFrame(df).to_csv('./results/dataframes/1d_exact_MMD_ABC/results_time.csv', index=False)