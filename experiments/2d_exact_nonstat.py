...
for m in [10, 50, 100, 500, 1000]:
    ...
    df['n'][index] = n
    df['m'][index] = m
    x_sample = lindley(service_rate=1, arrival_rate=1, service_shape_parameter=1, arrival_shape_parameter=0.5, replications=m, device=device)
    
    for k in tqdm(range(macro_count)):
        start_time = time.time()
        if m == 10:
            theta_tilde, _, para_list = sgd_estimation(lindley, x_sample, crps, theta_init, arrival_shape_parameter=0.5, simulation_replications=n, number_epochs=800, lr_init=1, device=device)
        else:
            theta_tilde, _, para_list = sgd_estimation(lindley, x_sample, crps, theta_init, arrival_shape_parameter=0.5, simulation_replications=n, number_epochs=800, lr_init=1, device=device)
        ...
        if k == 0:
            ...
            # Save the plot
            os.makedirs(f'../results/plots/2d_exact_nonstat', exist_ok=True)
            plt.savefig(f'../results/plots/2d_exact_nonstat/2d_exact_nonstat_m={m}_CI.png')
            plt.close()            
            ...
            # Save the plot
            os.makedirs(f'../results/plots/2d_exact_nonstat', exist_ok=True)
            plt.savefig(f'../results/plots/2d_exact_nonstat/2d_exact_nonstat_m={m}_conv.png')
            plt.close()
    ...
    os.makedirs(f'./results/dataframes/2d_exact_nonstat', exist_ok=True)

    # Save the dataframe to a CSV file
    pd.DataFrame(df).to_csv(f'./results/dataframes/2d_exact_nonstat/results_m={m}.csv', index=False)
...
# Create the directory if it doesn't exist
os.makedirs('./results/dataframes/2d_exact_nonstat', exist_ok=True)

# Save the dataframe to a CSV file
pd.DataFrame(df).to_csv(f'./results/dataframes/2d_exact_nonstat/results.csv', index=False)