...
for service_shape in [0.2]:
    ...
    for k in tqdm(range(macro_count)):
        ...
        if k == 0:
            ...
            plt.savefig(f'./results/plots/2d_inexact_stat/2d_inexact_stat_alpha={service_shape}_beta={beta}_CI.png')
            plt.close()            
            ...
            # Save the plot
            os.makedirs('./results/plots/2d_inexact_stat', exist_ok=True)
            plt.savefig(f'./results/plots/2d_inexact_stat/2d_inexact_stat_alpha={service_shape}_beta={beta}_conv.png')
            plt.close()
    ...
    pd.DataFrame(df).to_csv(f'./results/dataframes/2d_inexact_stat/results_alpha={service_shape}_beta={beta}.csv', index=False)
    index += 1
...
# Save the dataframe to a CSV file
pd.DataFrame(df).to_csv(f'./results/dataframes/2d_inexact_stat/results_beta={beta}.csv', index=False)