...
for m in [10, 100, 1000]:
    ...
    for k in tqdm(range(macro_count)):
        ...
        y_samples = torch.stack([lindley(service_rate=lambda_i[0], arrival_rate=lambda_i[1], arrival_shape_parameter=0.5, replications=N, device=device).to(device)
        ...
        if k == 0:
            ...
            plt.savefig(f'./results/plots/rejectionABC_2d_inexact/rejectionABC_2d_inexact_m={m}_CI.png')
            plt.close()   
            ...
            # Save the plot
            plt.savefig(f'./results/plots/rejectionABC_2d_inexact/rejectionABC_2d_inexact_posterior_serv_m={m}.png')
            plt.close()
            ...
            # Save the plot
            plt.savefig(f'./results/plots/rejectionABC_2d_inexact/rejectionABC_2d_inexact_posterior_arriv_m={m}.png')
            plt.close()
    ...
    pd.DataFrame(df).to_csv(f'./results/dataframes/rejectionABC_2d_inexact/results_m={m}.csv', index=False)
    index += 1
